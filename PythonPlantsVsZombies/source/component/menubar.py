...
# timer image cause I desperately need the practice, takes the time as a float in seconds
def getTimerValueImage(timer_value):
    font = pg.font.SysFont(None, 22)
    width = 80
    msg_image = font.render(f'{timer_value:.1f}', True, c.NAVYBLUE, c.LIGHTYELLOW)
    msg_rect = msg_image.get_rect()
    msg_w = msg_rect.width
    ...
    image.fill(c.LIGHTYELLOW)
    image.blit(msg_image, (x-25, 5), (0, 0, msg_rect.w, msg_rect.h))
    image.set_colorkey(c.BLACK)
    return image

def getRealTimeImage(dt):
    font = pg.font.SysFont(None, 22)
    width = 80
    time_image = font.render(dt, True, c.NAVYBLUE, c.LIGHTYELLOW)
    ...
    image.fill(c.LIGHTYELLOW)
    image.blit(time_image, (x-15, 5), (0, 0, time_rect.width, time_rect.height))
    image.set_colorkey(c.BLACK)
    return image
...
class MenuBar():
    ...
    # added timer, takes time in seconds as float
    def drawTimerValue(self, timer_value):
        self.value_image = getTimerValueImage(timer_value)
        self.value_rect = self.value_image.get_rect()
        self.value_rect.x = 5
        self.value_rect.y = self.rect.bottom - 50
        
        self.image.blit(self.value_image, self.value_rect)
    
    def drawRealTime(self, dt):
        self.value_image = getRealTimeImage(dt)
        self.value_rect = self.value_image.get_rect()
        self.value_rect.x = 5
        self.value_rect.y = self.rect.bottom - 75
        
        self.image.blit(self.value_image, self.value_rect)

    def draw(self, surface, timer_value, dt):
        self.drawSunValue()
        self.drawRealTime(dt)
        self.drawTimerValue(timer_value)
        surface.blit(self.image, self.rect)
        for card in self.card_list:
            card.draw(surface)
