...
class threadPlayMusic:
    def stopBackgroundMusic(self):
        pg.mixer.stop()
    def playZombieMusic(self):
        self.stopBackgroundMusic()  # Assuming you have a function to stop background music

        # Define the path to the mp3 files
        zombie_coming_mp3_path = os.path.join('resources', 'Bgm', 'zombieComing.mp3')
        zombie_greet_mp3_path = os.path.join('resources', 'Bgm', 'zombieGreet_bgm.mp3')

        # Initialize the mixer (if not already done)
        pg.mixer.init()

        # Load background music
        zombieComing_music = pg.mixer.Sound(zombie_coming_mp3_path)

        # Define a function to play the next music after the first one is done
        def playNextMusic():
            zombieGreet_music = pg.mixer.Sound(zombie_greet_mp3_path)
            zombieGreet_music.play()

        # Set up an event to trigger playing the next music
        pg.time.set_timer(pg.USEREVENT, int(zombieComing_music.get_length() * 1000))

        # Play the first background music
        zombieComing_music.play()

        # Define a function to play the next music after a delay
        def playNextMusicDelayed():
            pg.time.delay(int(zombieComing_music.get_length() * 1000))
            playNextMusic()

        # Start a separate thread for playing the next music
        threading.Thread(target=playNextMusicDelayed).start()
...
class Level(tool.State):
    ...
    # zombieGreet bgm.mp3
    def playZombieWaveMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join( 'resources','Bgm','Zombie_fristWave.mp3')
        # Initialize the mixer (if not already done)
        pg.mixer.init()
        # Load background music
        zombieWave_music = pg.mixer.Sound(mp3_path)
        # Play the background music (play it once)
        zombieWave_music.play()
...
# Call the playZombieMusic method
        obj.playZombieMusic()

    # planting bgm.mp3
    def playPointsGetMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join( 'resources','Bgm','points.mp3')
        # Initialize the mixer (if not already done)pg.mixer.init()
        # Load background music
        points_music = pg.mixer.Sound(mp3_path)
        # Play the background music (play it once)
        points_music.play()
    ...
    # planting bgm.mp3
    def playPlantingMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join( 'resources','Bgm','plant.mp3')
        # Initialize the mixer (if not already done)pg.mixer.init()
        # Load background music
        planting_music = pg.mixer.Sound(mp3_path)
        # Play the background music (play it once)
        planting_music.play()
    ...
    def addPlant(self):
        ...
        if self.plant_name == c.SUNFLOWER:
            ...
            new_plant = plant.WallNut(x, y,self.bullet_groups[map_y])
            self.playPlantingMusic()
    ...
    # zombieGreet bgm.mp3
    def playZombieEatingMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join( 'resources','Bgm','zombieEating.mp3')
        # Initialize the mixer (if not already done)pg.mixer.init()
        # Load background music
        zombieEating_music = pg.mixer.Sound(mp3_path)
        # Play the background music (play it once)
        zombieEating_music.play()
    ...
    # add car music
    # zombieGreet bgm.mp3
    def playCarCleanMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join( 'resources','Bgm','carClean.mp3')
        # Initialize the mixer (if not already done)
        pg.mixer.init()
        # Load background music
        carClean_music = pg.mixer.Sound(mp3_path)
        # Play the background music (play it once)
        carClean_music.play()
    ...
    def playVictoryMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join('resources', 'Bgm', 'victory_bgm.mp3')

        # Initialize the mixer (if not already done)
        pg.mixer.init()

        # Load background music
        victory_music = pg.mixer.Sound(mp3_path)

        # Play the background music (play it once)
        victory_music.play()

    def playFailMusic(self):
        # Define the path to the mp3 file
        mp3_path = os.path.join('resources', 'Bgm', 'fail_bgm.mp3')
    ...
    def draw(self, surface):
        dt = pend.now('local')
        dt.to_time_string()
        dt_formatted = dt.format('HH:mm A')
        self.level.blit(self.background, self.viewport, self.viewport)
        surface.blit(self.level, (0,0), self.viewport)
        if self.state == c.CHOOSE:
            self.panel.draw(surface)
        elif self.state == c.PLAY:
            self.menubar.draw(surface, (self.current_time - self.start_time) / 1000, dt_formatted)
            for i in range(self.map_y_len):
