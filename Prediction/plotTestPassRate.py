...
# Given data  
with open('predtionData_testPassRate.txt', 'r') as file:
...
cleaned_runtime_EWMA = [leadTime[i] for i in range(len(leadTime)) if len(outlier_indices_runtime) == 0 or i not in outlier_indices_runtime[0]]
cleaned_runtime_EWMA = [x * 100 for x in cleaned_runtime_EWMA]
...
regression_line = poly_linear(x_range)  # Calculate corresponding y values
plt.plot(x_range, regression_line, color='green', label='Linear Regression Line')
...
plt.scatter(next_days, predicted_CVSS, color='red', label='Predicted Test Pass Rate')

# Adding labels and legend
plt.xlabel('Days (Excluding Outliers)')
plt.ylabel('Test Pass Rate(%)')
plt.legend()
plt.title('Test Pass Rate (linear)')
...
plt.savefig('TestPassRate_linear.png')
...
predicted_values = poly_poly(x_range)

# Plotting the regression line
plt.plot(x_range, predicted_values, color='green', label='Polynomial Regression Line')
...
plt.ylabel('Test Pass Rate(%)')
plt.legend()
plt.title('Test Pass Rate Prediction (polynomial)')
# Set y-axis limit to start from 0
...
# Saving the plot as an image
plt.savefig('TestPassRate_polynomial.png')
...
plt.scatter(forecasted_x, forecasted_points, color='red', label='Predicted Test Pass Rate', marker='o')

# Adding labels and legend
plt.xlabel('Days (Excluding Outliers)')
plt.ylabel('Test Pass Rate(%)')
plt.legend()
plt.title('Test Pass Rate Prediction (EWMA)')
...
# Saving the plot as an image
plt.savefig('TestPassRate_wma.png')
