...
# Given data  
with open('predictionData_Runtime.txt', 'r') as file:
    lines = file.readlines()
runtime = []

for line in lines:
    freq_part = line.split(' = ')
    runtime.append(float(freq_part[1]))
...
z_scores_deployment = z_score(runtime)
# Get indices of outliers for both datasets
outlier_indices_runtime = np.where(np.abs(z_scores_deployment) > threshold)
x_values = [day for i, day in enumerate(range(1, len(runtime) + 1)) if i not in outlier_indices_runtime[0]]

# Remove outliers from both datasets
cleaned_runtime_EWMA = [runtime[i] for i in range(len(runtime)) if i not in outlier_indices_runtime]
# for polynomial
...
# Plotting the data points in the original order
plt.scatter(x_values, cleaned_runtime_linear, color='blue', label='Data Points')
...
plt.plot(cleaned_runtime_linear, poly_linear(cleaned_runtime_linear), color='green', label='Linear Regression Line')

# Plotting the predicted points
plt.scatter(next_days, predicted_CVSS, color='red', label='Predicted SAST runtime')

# Adding labels and legend
plt.xlabel('Days (Excluding Outliers)')
plt.ylabel('SAST Runtime')
plt.legend()
plt.title('SAST Runtime Prediction (linear)')
...
plt.savefig('SASTruntime_linear.png')
...
deploy_linspace = np.linspace(np.min(cleaned_runtime_polynomial), np.max(cleaned_runtime_polynomial), 50)
#plt.plot(cleaned_runtime_polynomial, poly_poly(cleaned_runtime_polynomial), color='green', label='Polynomial Regression Line')
...
plt.ylabel('SAST Runtime')
plt.legend()
plt.title('SAST Runtime Prediction (polynomial)')
# Set y-axis limit to start from 0
max_value = max(cleaned_runtime_polynomial)
max_value_predict = max(predicted_CVSS)
chosen_max_value = max(max_value, max_value_predict)
upper_limit = chosen_max_value + 10
plt.ylim(0, upper_limit)
...
# Saving the plot as an image
plt.savefig('SASTruntime_polynomial.png')
...
plt.scatter(forecasted_x, forecasted_points, color='red', label='Predicted SAST runtime', marker='o')

# Adding labels and legend
plt.xlabel('Days (Excluding Outliers)')
plt.ylabel('SAST Runtime')
plt.legend()
plt.title('SAST Runtime Prediction (EWMA)')
...
# Saving the plot as an image
plt.savefig('SASTruntime_wma.png')
